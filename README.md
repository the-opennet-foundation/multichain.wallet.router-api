...
```sh
npm install git+ssh://git@github.com:stainless-sdks/multichain.wallet.router-api-typescript.git
```
...
const response: MultichainWalletRouterAPI.HealthCheckResponse = await client.health.check();
```
...
const response = await client.health.check().catch(async (err) => {
  if (err instanceof MultichainWalletRouterAPI.APIError) {
...
1. Via the `MULTICHAIN_WALLET_ROUTER_API_LOG` environment variable
...
const client = new MultichainWalletRouterAPI({
  logger: logger.child({ name: 'MultichainWalletRouterAPI' }),
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/multichain.wallet.router-api-typescript/issues) with questions, bugs, or suggestions.
