...
import { APIPromise } from '@paxeer-network/multichain.wallet.router-api/core/api-promise';

import util from 'node:util';
import MultichainWalletRouterAPI from '@paxeer-network/multichain.wallet.router-api';
import { APIUserAbortError } from '@paxeer-network/multichain.wallet.router-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['MULTICHAIN_WALLET_ROUTER_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: MultichainWalletRouterAPI) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new MultichainWalletRouterAPI({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['MULTICHAIN_WALLET_ROUTER_API_LOG'] = 'not a log level';
      const client = new MultichainWalletRouterAPI({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'MULTICHAIN_WALLET_ROUTER_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['MULTICHAIN_WALLET_ROUTER_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new MultichainWalletRouterAPI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['MULTICHAIN_WALLET_ROUTER_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['MULTICHAIN_WALLET_ROUTER_API_BASE_URL'] = ''; // empty
      const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://wallet.api-router.paxeer.app/api');
    });

    test('blank env variable', () => {
      process.env['MULTICHAIN_WALLET_ROUTER_API_BASE_URL'] = '  '; // blank
      const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://wallet.api-router.paxeer.app/api');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['MULTICHAIN_WALLET_ROUTER_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new MultichainWalletRouterAPI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['MULTICHAIN_WALLET_ROUTER_API_API_KEY'] = 'My API Key';
    const client = new MultichainWalletRouterAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['MULTICHAIN_WALLET_ROUTER_API_API_KEY'] = 'another My API Key';
    const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new MultichainWalletRouterAPI({ apiKey: 'My API Key', fetch: testFetch });
